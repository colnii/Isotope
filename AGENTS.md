...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `883 passed`.
...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, submit action helper, and artifact review flow first slices are complete; Artifact Review Flow Friction Review is complete; next suggested batch is Source Artifact Setup Helper.
