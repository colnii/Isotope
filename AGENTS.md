...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `898 passed`.
...
- Demo trace mode is implemented for `--scenario v0.2 --trace`, `--scenario approval-tool-runner --trace`, and `--scenario artifact-review --trace`; it is human-readable only, keeps `--json` compatible, and does not expose artifact full content.
...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, submit action helper, artifact review flow, source artifact setup helper, and artifact provenance helper first slices are complete; artifact-review first app spike is closed for now; second app spike selection recommends `external snapshot review`; next suggested batch is External Snapshot Review Red Tests.
