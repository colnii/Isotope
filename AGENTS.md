...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `865 passed`.
...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, and submit action helper first slices are complete; usability friction round 1 is closed; next suggested batch is artifact review flow red tests only.
