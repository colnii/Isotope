...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; current integration baseline is `1379 passed, 5 skipped`; pre-controlled-terminal mainline baseline was `1134 passed`.
...
- Capability Search / Launch Plan Boundary is documented in [docs/capability-search-launch-plan-boundary-v0.2.md](docs/capability-search-launch-plan-boundary-v0.2.md). The next implementation slice should define deterministic catalog search and low-sensitive launch plans before execution; it must not call an LLM router, construct providers, execute capabilities, append events, open `ask` / `interactive`, or copy aggressive hub code.
