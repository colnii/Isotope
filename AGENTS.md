...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, submit action helper, and artifact review flow first slices are complete; next suggested batch is Artifact Review Flow Friction Review.
