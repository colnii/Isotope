...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. `Approval Tool Runner API Friction Review` is complete and documented in [docs/approval-tool-runner-friction-review.md](docs/approval-tool-runner-friction-review.md); next suggested batch is `Approval Lookup Helper Boundary`, pending user confirmation.
