...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `806 passed`.
...
- Post-tag delta is documented in [docs/post-v0.2-tag-delta.md](docs/post-v0.2-tag-delta.md): current `main` is ahead of `v0.2-demo` with Track F external ingestion boundary work, Agent / Worker lifecycle first slice, and Workspace substrate first slice; do not move the tag or create `v0.2.1-demo` unless explicitly requested.
