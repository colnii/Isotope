...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; current integration baseline is `1362 passed, 5 skipped`; pre-controlled-terminal mainline baseline was `1134 passed`.
...
- Capability Runner Thin Shell boundary is documented in [docs/capability-runner-thin-shell-boundary-v0.2.md](docs/capability-runner-thin-shell-boundary-v0.2.md). Future implementation must reuse `capability_catalog` as source of truth, support only a tiny allowlist of existing product-candidate capabilities, and fail closed before side effects. Do not copy aggressive `capability_hub.py`, do not add workflow engine / product shell / study companion / self-evolution harness / real provider runner in this slice.
