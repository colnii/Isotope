...
- Capability Hub Core Boundary is documented in [docs/capability-hub-core-boundary-v0.2.md](docs/capability-hub-core-boundary-v0.2.md). Mainline should not merge aggressive Capability Hub wholesale; only a small extracted catalog core is in scope: metadata model, shelf visibility, minimal list / manifest / status, and a few product-candidate registrations. Do not copy aggressive `capability_hub.py`, diagnostics, self-evolution harness, DeepSeek provider, LLM route, `ask`, `interactive`, workflow engine, or product shell into mainline.
