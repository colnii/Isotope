...
- `v0.1-demo` developer demo is accepted; baseline is `630 passed`.
...
- Track A has in-process `HttpApiApp` / `create_http_app(...)`, request validation / no-side-effect error boundary, response contract, demo smoke, and duplicate-submit idempotency boundary; it is not a real listening HTTP server.
