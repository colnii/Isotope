...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). Current Batch is complete; next suggested batch is `Kernel Usability Pressure Test Planning`, but do not start it without an updated queue or explicit user instruction.
