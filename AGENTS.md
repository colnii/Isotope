...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `892 passed`.
...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, submit action helper, artifact review flow, and source artifact setup helper first slices are complete; source artifact helper closure review is closed; next suggested batch is Artifact Review Flow Second Friction Review.
