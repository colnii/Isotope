...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). It uses rolling batch mode with a 45-60 min session timebox. Approval lookup, workspace binding, submit action helper, artifact review flow, source artifact setup helper, artifact provenance helper, and external snapshot review first slices are complete; artifact-review first app spike is closed for now; next suggested batch is External Snapshot Review Friction Review.
