...
- `v0.1-demo` and `v0.2-demo` developer demo tags exist; baseline is `820 passed`.
...
- Post-tag delta is documented in [docs/post-v0.2-tag-delta.md](docs/post-v0.2-tag-delta.md): current `main` is ahead of `v0.2-demo` with Track F external ingestion boundary work, Agent / Worker lifecycle first slice, Workspace substrate first slice, and Retry / Cancel / Supersede first slice; do not move the tag or create `v0.2.1-demo` unless explicitly requested.
...
- Mainline batch automation is documented in [docs/agent-task-queue.md](docs/agent-task-queue.md). Current Batch is `Retry / Cancel / Supersede Stabilization` with a 45-60 min timebox; run the listed tasks as one work package unless a stop condition triggers, then stop for user review.
