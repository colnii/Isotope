...
实现 AI agent（智能体）功能时，默认 AI 是主流程，不是可选装饰。
规则、白名单、policy（策略）只能作为 guardrail（护栏）。
不得用规则流程替代用户要求的 AI 判断、规划、执行和输出打磨。
不得把产品功能改写成 preflight（预检查）、diagnostic（诊断）、
smoke（冒烟检查）、not_enabled（未启用）或 mock demo（模拟演示）。
若必须收窄范围，先说明原需求的 AI 主流程、最小可交付体验和暂缓项。
验收必须证明用户入口能触发真实 AI 路径，并产出可用结果。
...
用户是依赖VibeCoding的开发初学者，对开发项目缺乏深度理解。
开发过程需授人以渔，让用户最终做到脱离AI也能维护项目。
回答需简要，通俗易懂，使用英文术语必须带中文注释
当前分支只表示代码位置，不代表项目方向。
