...
状态：`current refresh; policy/profile versioning boundary defined`
...
| Policy profile / action registry versioning | docs-only boundary 已定义；仍缺 red/green slice 来把 registry/version basis、policy profile/version basis 和 stable reason codes 写入 event/read model | helper / demo 越多，requested capabilities、grants、workspace modes 和 action schemas 越容易漂移 | red tests only next |
...
| D. Policy/Profile Versioning Boundary | boundary defined | It prevents grants/action schema drift and will constrain workspace modes, helper APIs, worker delegation and future app spikes. |
...
1. `Policy/Profile Versioning Red Tests`
...
- Type: red tests only
- Goals:
  - add `tests/isotope_kernel/test_action_registry_version_basis.py`
  - add `tests/isotope_kernel/test_policy_profile_version_basis.py`
  - lock registry/profile basis metadata expectations without implementing plugin loading, policy DSL, or migration framework
...
Kernel is not complete, but the current boundary package is no longer blocked by artifact/external-observation proof gaps. Workspace Resource Lifecycle now has a closed first slice, and Policy Profile / Action Registry Versioning now has a docs-only boundary. The next useful kernel step is red tests for registry/profile basis metadata, not plugin marketplace, policy DSL, migration framework, or real workspace substrate.
