...
归档原因：本文保留旧任务队列的完整历史批次。当前可执行队列已经收缩到
[agent-task-queue](../../current/agent-task-queue.md)，避免 `docs/current/`
入口继续承载 1400+ 行历史流水。
...
24. 命名与目录审计：已写 [naming-and-structure-review](../../current/naming-and-structure-review.md)。
25. 外部审查吸收：审查原文已归档到
    [chatgpt审查](./chatgpt审查.md)，当前执行入口是
    [import-map](../../current/import-map.md)。
...
33. 兼容代理审计：显式测试导入已切到新路径，旧代理清单见
    [compat-proxy-audit](../../current/compat-proxy-audit.md)。
...
76. Codex Supervisor 能力地图：已新增
    [supervisor-capability-map](../../current/supervisor-capability-map.md)，登记
...
180. Codex Supervisor 持久目标队列：新增 `goal add/list/archive`，
...
182. Codex Supervisor 目标状态可见化：`goal list` 和 `daemon status`
    ...
    手动翻 `goals.jsonl`。
183. Codex Supervisor daemon fanout / 低水位配置透传：`daemon start` 和
    `up` 支持 `--max-fanout-launches`、`--goal-low-water` 和
...
- 同步 [status](../../current/status.md)、[terminology](../../current/terminology.md)、
  [codex-supervisor-readonly](../../current/codex-supervisor-readonly.md) 和
  [supervisor-capability-map](../../current/supervisor-capability-map.md)。
...
- 当前仍只改变建议，不自动发送新指令。
- 同步 [status](../../current/status.md)、[terminology](../../current/terminology.md) 和
  [supervisor-capability-map](../../current/supervisor-capability-map.md)。
...
- 新增 [supervisor-capability-map](../../current/supervisor-capability-map.md)。
...
- 明确后续顺序：先做 tmux bell 信号，再做状态协议、lane state
  和 LLM planner 决策。
- 同步 [status](../../current/status.md)、[docs-map](../../current/docs-map.md)、
  [terminology](../../current/terminology.md) 和
  [codex-supervisor-readonly](../../current/codex-supervisor-readonly.md)。
...
- 同步 [application-structure-plan](../../current/application-structure-plan.md)、
  [terminology](../../current/terminology.md)、[status](../../current/status.md) 和
  [codex-supervisor-readonly](../../current/codex-supervisor-readonly.md)。
