...
- 来源：`docs/current/supervisor-command-reference.md`。
- 归档时间：2026-05-24。
- 归档原因：本文曾在 current 同时维护能力清单、命令示例、web 行为、
  daemon/runbook 和边界说明，篇幅长且随 Supervisor 高频变更，容易在多分支合入时
  产生 rebase 冲突，也容易和 `supervisor-operations-runbook.md`、
  `supervisor-capability-inventory.md` 重复。
- 当前事实：命令入口和更新规则以
  `docs/current/supervisor-command-reference.md` 为准；长流程以
  `docs/current/supervisor-operations-runbook.md` 为准；能力细节以
  `docs/current/supervisor-capability-inventory.md` 和
  `docs/current/supervisor-capability-details.md` 为准。
...
本文从原 `codex-supervisor-readonly.md` 拆出，保留完整命令说明、运行方式和
命令边界。长流程运行手册见
[Supervisor operations runbook](../../current/supervisor-operations-runbook.md)，短入口见
[Codex Supervisor 监控与托管](../../current/codex-supervisor-readonly.md)。

原状态：`第二版小切片 / LLM 管理层 + 本机监控 + 显式控制通道`

能力登记和后续拆分边界见
[Codex Supervisor 能力地图](../../current/supervisor-capability-map.md)。
...
详细夜间 smoke、`supervise` 小闭环、LLM summary、托管登记和状态协议已拆到
[Supervisor operations runbook](../../current/supervisor-operations-runbook.md)。
本文件只保留命令入口、运行方式和边界说明，降低日常命令更新与长流程验收文档的冲突面。
