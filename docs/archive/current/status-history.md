...
归档原因：本文保留旧 status 的完整历史状态和验证命令。当前状态入口已经收缩到
[status](../../current/status.md)，避免历史流水和当前事实并列造成误读。
...
19. `features/supervisor` 已有 Codex Supervisor 监控与托管启动，
    ...
    输出中文汇报；`start-here` 可打印第一次试用的最短路径，
    包含启动后台、打开页面、查看状态、反馈观察点和停止后台；
    `watch --changes-only` 可只在变化时再次输出；
    ...
    改动摘要、pytest gate 结果、建议验证命令、复查提示（reviewer prompt）和可复制
    `codex exec -C ...` 复查命令；该入口只做高可信审查汇总，
    不自动合并、不删除 worktree 或分支；
    `integration-review` 可只读扫描 managed worker 的 branch、
    worker HEAD、`main` 是否已包含、pytest gate、merge-tree 冲突风险和
    ...
    长跑时反复启动同名后台任务；process worker 已退出且最后明确汇报过
    `SUPERVISOR_STATUS: working` 时，`loop` 会按 lane state 里的
    ...
    低水位按“可推进目标”计算，已 `done/blocked/needs_user` 或已有同名
    worker 正在运行的目标不会继续占补给水位；
    如果低水位 goal plan 返回 `parallel_recommendations`，同轮 fanout
    会优先按模型推荐的目标批次启动受控 `launch_session`；
    ...
    web 已新增“运行焦点”区，放在页面顶部汇总后台循环、
    需要看的 Codex 窗口、工作中的 Codex 窗口、当前目标和前三个重点项；
    该区会优先显示当前 Web 工作区内的 Codex 窗口，避免跨仓库历史会话抢占首屏，
    让人类先看到运行结论，再下钻看 Worker 详情；
