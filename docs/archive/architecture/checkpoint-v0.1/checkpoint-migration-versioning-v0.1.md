...
- checkpoint schema version fields design note 已落文档，边界见 `checkpoint-schema-version-fields-v0.1.md`。
- event envelope 仍是 slice-only shape，不是最终协议。
- event envelope versioning design note 已落文档，最小 event envelope version boundary 已实现；边界见 `event-envelope-versioning-v0.1.md`。
- event envelope schema registry design note 已落文档，边界见 `event-envelope-schema-registry-v0.1.md`。
...
- checkpoint history / old-checkpoint fallback design note 已落文档，边界见 `checkpoint-history-fallback-v0.1.md`。
...
- checkpoint schema version fields 的详细边界见 `checkpoint-schema-version-fields-v0.1.md`。
...
`event_envelope_version` 的更完整边界见 `event-envelope-versioning-v0.1.md`。当前 event representation version 是 `canonical_event_slice@v0`，但这仍只是当前 slice implementation shape，不是最终 protocol。
...
- future event envelope schema version 变化后，event prefix digest 如何迁移或比较；当前边界见 `event-envelope-versioning-v0.1.md`。
