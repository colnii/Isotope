...
本文定义 checkpoint history index（检查点历史索引）和 retention policy（保留策略）的 v0.1 边界。当前只落设计说明，不实现 history index、retention policy、GC 或 `CheckpointService`。checkpoint v0.1 已按 `checkpoint-v0.1-scope-freeze.md` frozen for current kernel slice；本文不是下一步实现计划。
...
- checkpoint history save boundary design note 已落文档，见 `checkpoint-history-save-boundary-v0.1.md`。
- checkpoint history save integration boundary design note 已落文档，见 `../reviews/checkpoint-history-save-integration-v0.1.md`。
...
- checkpoint v0.1 scope freeze 已落文档，见 `checkpoint-v0.1-scope-freeze.md`。
...
history save path 如果保留 checkpoint history，必须先遵守 `checkpoint-history-save-boundary-v0.1.md`，不能静默改变 current latest-only save behavior。
