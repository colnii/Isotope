...
- event envelope versioning design note 已落文档，边界见 `event-envelope-versioning-v0.1.md`。
...
digest metadata 记录 event representation version。字段名仍是当前 v0 implementation shape，边界见 `event-envelope-versioning-v0.1.md`。
...
- checkpoint migration / version negotiation 对 digest metadata 的影响；边界见 `checkpoint-migration-versioning-v0.1.md`。
- event envelope schema registry 引入后 digest 输入如何版本化；边界见 `event-envelope-versioning-v0.1.md`。
