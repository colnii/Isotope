...
- `ActionTypeRegistry` 已由 Action Compiler 消费用于 compact tool lookup、由 Policy Engine 用于 requirement lookup、由 Executor 用于 handler lookup，并由 InProcessServer 以 shared registry wiring 贯穿三者；registry schema / 版本化仍未定，当前最小边界见 `action-type-registry-v0.1.md`。
...
当前 v0 slice 已有 `MemoryRecord` implementation shape，用于验证 structured content、source refs 和 execution/action provenance；executor memory handler not-enabled / provenance boundary 也已落地：authorized `write_memory` 在显式 `memory_service` 下会构造 record 并把 runtime execution provenance / grants 交给 memory service，但当前 not-enabled service 仍拒绝写入。memory record persistence not-enabled boundary 也已实现为 `NotEnabledMemoryStore`：它只拒绝保存并提供 empty `list_records(...)` / path locator，不写文件、不产生 success event、不留下 partial record。agent-loop memory integration 现在有一个窄的本地 slice：`record_turn_memory` 通过 completed `write_memory` action、`memory.record_created` canonical event 和平台层 `FileMemoryStore` 保存 structured content；`query_memory` 只返回 summary / refs / provenance / quality，并能在 server restart 后检索。memory query authorization not-enabled boundary 仍作为 fail-closed fallback 保留。`memory.record_created` canonical event read-model boundary 已实现：projector 只从 canonical event 投影 `RunState.memory_records` summary / refs / provenance metadata，不读取 memory store，不投影 full content，并要求 event 绑定 completed `write_memory` execution。`memory.record_superseded` canonical event read-model boundary 也已实现：supersession 只能通过 append-only canonical event 表达，旧 record 不被覆盖，只增加 supersession metadata 并指向已存在的新 record，且 event 必须绑定 completed `write_memory` execution。memory read-model checkpoint boundary 也已实现：checkpoint state 可包含 `memory_records`，checkpoint-assisted rebuild 可恢复该 read model 并 replay suffix events，schema / prefix consistency 会校验 memory record shape、supersession metadata 和 full-content 禁止字段。controlled expand 已打开 matched `MemoryRecord.content` 的 budgeted materialization 第一片；当前仍不代表 embedding / ranking / source artifact full-content expand / automatic promotion policy / full product memory 已实现。
...
- `ActionTypeRegistry` 的 schema 和版本化；compiler lookup、policy requirement lookup、executor handler lookup 与 InProcessServer shared registry wiring 已实现，当前最小边界见 `action-type-registry-v0.1.md`。
