...
- `ActionTypeRegistry` 的 schema、版本化、注册生命周期；当前最小边界见 `action-type-registry-v0.1.md`。
...
当前 v0 slice 已有 `MemoryRecord` implementation shape，用于锁住 structured content / provenance / refs 的最小边界；executor memory handler not-enabled / provenance boundary 已落地，会在显式 `memory_service` 下把 authorized `write_memory` 转成带 runtime execution provenance 的 record 调用，但当前 not-enabled service 仍拒绝写入。memory record persistence not-enabled boundary 已实现为 `NotEnabledMemoryStore`，只做受控拒绝、empty listing 和 path locator，不写文件、不产生 success event、不留下 partial record。agent-loop memory integration 已提供一个窄的本地 slice：`record_turn_memory` 通过 completed `write_memory` action、`memory.record_created` canonical event 和平台层 `FileMemoryStore` 保存 structured content；`query_memory` 只返回 summary / refs / provenance / quality，并可在 server restart 后从本地 store 检索。memory query authorization not-enabled boundary 仍保留为 fail-closed fallback。`memory.record_created` canonical event read-model boundary 已实现：`RunState.memory_records` 只从 canonical event 投影 summary / refs / provenance metadata，拒绝 full content 字段，并要求绑定 completed `write_memory` execution。`memory.record_superseded` canonical event read-model boundary 也已实现：memory update 语义只能通过 append-only supersession event 表达，旧 record 不被原地覆盖，只增加 supersession metadata 并指向已存在的新 record，且 event 必须绑定 completed `write_memory` execution。memory read-model checkpoint boundary 也已实现：`RunProjector.create_checkpoint(...)` 会包含 `memory_records`，`RunProjector.rebuild_with_checkpoint(...)` 可从 checkpoint + suffix events 恢复该 read model，schema / prefix consistency 会校验 memory record shape、supersession metadata 和 full-content 禁止字段。memory promotion proposal boundary 已补第一片：只从 structured artifact metadata 或 accepted external observation metadata 生成待批准的 `write_memory` `ActionProposal`，拒绝 raw text / raw content 直接进入长期 memory。agent-loop 另有 explicit `promote_run_memory` run -> same-session 晋升，controlled expand 也已打开 matched `MemoryRecord.content` 的 budgeted materialization 第一片；当前仍不代表 embedding / ranking / source artifact full-content expand / automatic promotion policy / full product memory 已实现。
