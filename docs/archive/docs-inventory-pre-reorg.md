...
- 加上 Capability Search / Launch Plan Boundary 后 tracked docs Markdown：150 个。该文档定义 capability search / launch plan preflight contract，明确 search 可先 deterministic，但长期稳定接口是 low-sensitive launch plan；当前不实现 LLM router、provider runner、workflow engine 或 self-evolution。
- Capability Search / Launch Plan first slice 已 green；tracked docs Markdown 数量不变。当前实现新增 `CapabilityRunner.search_capabilities(...)` 和 `plan_capability_run(...)`，只做 no-side-effect Python API，不实现 CLI `search` / `plan`、LLM router、provider runner、workflow engine 或 product shell。
...
- `docs/capability-search-launch-plan-boundary-v0.2.md`：Capability Search / Launch Plan boundary，记录 search / preflight / launch-plan output shape、no-side-effect rules、未来 LLM router 关系和 first green slice evidence。
