...
它的目标是帮助外部读者快速理解 `python -m isotope.demo` 跑通了什么：一个本地 deterministic core loop（确定性核心闭环），从 input 到 action chain，再到 artifact、canonical events、projector、checkpoint 和 replay verification。
