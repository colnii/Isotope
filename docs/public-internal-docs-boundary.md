...
| --- | --- | --- | --- |
| `public-ready` | 可以直接给外部读者看，语气和内容接近公开项目说明。 | `README.md`, `docs/demo/`, `docs/release/`, `docs/external-review-package-v0.2.md`, `docs/post-external-review-checkpoint.md` | 可默认包含，但 release 前仍要检查 stale status / tag wording。 |
| `reviewer-facing` | 适合给技术 reviewer 看，包含实现边界、已证明能力、deferred surfaces。 | `docs/current-status.md`, `docs/kernel-mainline-maintenance-mode.md`, `docs/app-spike-coverage-review.md`, `docs/kernel-gap-review-refresh-v0.2.md`, closure reviews | 可公开，但需要说明它们是 design / review records，不是 product docs。 |
| `internal/dev-process` | 服务于开发流程、AI 协作、队列和批次推进。 | `AGENTS.md`, `docs/agent-task-queue.md`, detailed red/green/closure docs | 可留在 repo，但不应被包装成 public product narrative。 |
| `concept/application-pressure` | 长期概念、应用层设想、参考项目对照、study companion pressure。 | `docs/concepts/`, selected `docs/kernel-living-spec.md` sections | 当前可留在主线，但未来公开前需要逐篇 audit。 |
...
- 当前实现事实仍以 `docs/current-status.md`、`docs/v0.2-roadmap.md`、`docs/agent-task-queue.md`、closure reviews 和 actual code / tests 为准。
...
- Do not treat `docs/agent-task-queue.md` as public product documentation.
