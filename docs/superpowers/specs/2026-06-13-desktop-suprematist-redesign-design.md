...
- 视觉引用至上主义的画布暖白、红黄蓝黑几何块和非对称构图。
- 装饰不能抢走内容，不做海报化全屏背景、大面积渐变、纹理背景或纯艺术化界面。
...
本设计定义整套 desktop 前端的长期视觉语言；第一轮实现只覆盖主聊天黄金路径，
后续再把同一套 token 和组件规则扩展到其他桌面界面。
...
- 保持单列对话主轴。
- 顶部只显示模式、标题和必要状态。
- 顶部右侧需要分清两层：几何品牌构成独占角落，状态 chip 进入信息区或
  明确避让，不能出现 `turns`/状态文字遮住几何元素。
...
- `canvas`: 接近至上主义画布白的暖白主背景，避免牛皮纸感和复古旧纸感。
- `panel`: 内容纸面，略高于背景。
- `ink`: 近黑主文本；黑色几何块可以出现，但不作为粗重边框主材。
- `muted`: 灰褐辅助文字。
- `line`: 浅米灰细线，用于卡片、输入和区域边界。
- `red`: 品牌几何标记、发送等关键动作，也用于错误和拒绝，但错误态必须有文案与 icon/标记共同区分。
- `yellow`: 等待、注意、审批。
- `blue`: 用户消息、主要执行状态、正在运行的能力。
- `green`: 仅少量用于完成状态，不成为主视觉家族。

形态：

- 控件以 6-8px 小圆角或直角为主，保持现代工具感。
- 边界以浅细线为主，避免粗黑线把界面变成复古海报。
- 黑色只作为少量几何标记或强文本出现，不支配整个骨架。
- 状态强调用小几何块、侧边色条、角标、斜切/错位小块，而不是整卡染色。
- 标题区可以使用非对称红/蓝/黄/黑几何构成，但必须为状态 chip、
  标题和窗口控制预留独立空间，不能互相遮挡。
- 不使用离散装饰圆球、bokeh、紫蓝 AI 渐变、玻璃拟态、纹理背景或
  大面积渐变。至上主义引用应来自平面构成和留白，不来自背景特效。
...
- 主动作优先使用红色或蓝色；红色用于发送/品牌关键动作时，错误态必须用
  文案、图标或几何标记进一步区分，避免正常动作被误读为危险。
- 危险/拒绝使用红色。
...
- 卡片边界清晰，背景以 canvas/panel 为主。
- 卡片边框使用浅细线；需要强调时先用侧条、角标或小几何块，而不是加粗整卡边框。
- 重要卡片可使用左侧色条或几何角标。
...
- 状态 chip、计数、模型名等低优先级信息必须让位于标题、消息和能力结果，
  不能压住几何标记或造成重叠。
