...
Desktop chat remains the user-facing entrypoint. The model sees concise
extension capabilities in `capacity_manifest`, then chooses whether to discover
skills, describe a specific skill, list MCP servers, search MCP tools, or call an
MCP tool.
...
directories, return every valid skill as metadata, and read a selected skill body
only after the model asks for details. This verifies that importing all current
...
The baseline `capacity_manifest` must contain only concise discovery metadata.
Skill bodies, full MCP result text, secrets, raw transcripts, raw JSON-RPC logs,
and complete embedded resources must not be injected into ordinary chat context.
...
- all current local Codex skills can be imported as metadata in a live/local
  smoke command without injecting every body into the prompt;
