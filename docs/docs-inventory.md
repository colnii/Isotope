...
当前自动推进入口是 `docs/agent-task-queue.md`。`Approval-Gated Tool Runner Spike` 已完成；不要自动进入 next suggested batch，先等用户 review exposed API friction。

- `docs/agent-task-queue.md`：active queue，Current Batch complete；Next Suggested Batch is API friction review after user review。
- `docs/usability-pressure-test-plan-v0.2.md`：current pressure-test planning doc，`approval-gated tool runner` first slice complete。
