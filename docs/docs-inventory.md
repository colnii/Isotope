...
- `docs/agent-task-queue.md`：active queue，Current Batch complete；Next Suggested Batch is `Retry / Cancel / Supersede Runtime Integration Boundary` with `ready_docs_only` status。
...
- `docs/kernel-gap-review-refresh-v0.2.md`：current kernel gap refresh；after policy/profile first slice closure, recommends Retry / Cancel / Supersede Runtime Integration Boundary next。
