...
- 加上 Retry / Cancel / Supersede Runtime Closure Review 后 docs Markdown：86 个。R/C/S runtime integration first slice 已 complete / closed for now；当前建议下一步做 Event Schema Registry / Compatibility Boundary，不实现 scheduler、process kill、real concurrency、plugin marketplace、policy DSL 或 migration framework。
...
- `docs/agent-task-queue.md`：active queue，Current Batch complete；Next Suggested Batch is `Event Schema Registry / Compatibility Boundary` with `ready_docs_only` status。
...
- `docs/kernel-gap-review-refresh-v0.2.md`：current kernel gap refresh；after policy/profile first slice closure and RCS runtime closure review, recommends Event Schema Registry / Compatibility Boundary next。
