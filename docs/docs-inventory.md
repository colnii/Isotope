...
- 加上 Event Schema Registry / Compatibility Boundary 及旧路径 compatibility stub 后 docs Markdown：88 个。当前建议下一步做 Event Schema Registry / Compatibility Red Tests，不实现 JSON Schema / protobuf / schema migration framework / plugin event system。
...
- `docs/kernel-gap-review-refresh-v0.2.md`：app spike 后的 kernel gap refresh，记录 first-slice enough surfaces、still-open kernel gaps 和 Event Schema Registry / Compatibility Red Tests next-step recommendation。
- `docs/event-schema-registry-compatibility-boundary-v0.2.md`：Event schema registry / compatibility boundary，记录 payload schema version、unknown event fail-closed target、compatibility rules 和 first red tests recommendation。
...
- `docs/agent-task-queue.md`：active queue，Current Batch complete；Next Suggested Batch is `Event Schema Registry / Compatibility Red Tests` with `ready_red_only` status。
...
- `docs/kernel-gap-review-refresh-v0.2.md`：current kernel gap refresh；after policy/profile first slice closure, RCS runtime closure review, and event schema compatibility boundary, recommends Event Schema Registry / Compatibility Red Tests next。
...
| `docs/event-schema-registry-compatibility-boundary-v0.2.md` | Event schema registry / compatibility boundary | boundary defined / red tests next |
