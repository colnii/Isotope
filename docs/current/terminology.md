...
| `ResearchProvider` | Research provider contract，封装 fake 或 Codex delegated research backend；provider registry 已列出 fake/codex/tavily/searxng/browser，未实现 provider fail closed；Codex backend 会提取 JSONL agent message，error-only JSONL 会归类为 provider failure，并带低敏 diagnostics，不写成功 report | 产品功能/外部研究 | `src/isotope/features/research/providers.py` |
