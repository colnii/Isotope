...
| `isotope-screen` | Screen 手动 smoke 命令行入口，用于本机窗口 observe/control 验收；`control-click` 可生成 click dry-run plan，`real-smoke-plan` 输出真实 backend 验证命令，`inspect/report` 可读取 screen artifact 或生成低敏 observe 摘要，`--allow-app` / `--allow-title-contains` 可注入命令级 target allowlist；不属于默认 CI | 应用入口/本机控制 | `src/isotope/features/screen/runner.py`, `src/isotope/features/screen/artifacts.py` |
