...
| `supervisor_action` | Supervisor 对外统一动作出口；固定生命周期动作可由程序直接生成，模糊动作再由 LLM planner 生成；旧 `llm_action` 只是兼容镜像 | 产品功能/控制策略/输出字段 | `src/isotope/features/supervisor/commands/supervisor_action.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
| `supervisor_action_planner` | `loop --json` / dashboard payload 中的动作来源说明，`source=program` 表示程序固定流程已决定，`source=llm` 表示进入 LLM fallback | 产品功能/控制策略/输出字段 | `src/isotope/features/supervisor/commands/supervise/program_action.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
| `LLM planner` | 大模型规划器，只处理程序无法确定的受控动作，例如目标拆解、上下文请求、新 worker prompt、恢复哪个会话或是否 `ask_user` | 产品功能/模型/控制策略 | `src/isotope/features/supervisor/llm_action/llm_summary.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
...
| `loop` | Supervisor 日常常驻入口，默认先执行程序可确定的固定流程，再把模糊动作交给 LLM planner fallback | 产品功能/控制通道 | `src/isotope/features/supervisor/supervise/payload.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
