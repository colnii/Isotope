...
| `ResearchFlow` | Research 功能入口，把 delegated web research 结果保存成结构化 report/raw transcript artifacts；provider 失败时保存 provider trace，成功 search 写 report | 产品功能/外部研究 | `src/isotope/features/research/flow.py` |
...
| `ResearchProvider` | Research provider contract，封装 Codex delegated backend 或 network-gated Tavily provider；provider registry 已列出 codex/tavily/searxng/browser，Tavily 缺配置或网络开关待批准时写 `research.provider_trace`，真实响应会归一化为 source-backed report；Codex backend 会提取 JSONL agent message，error-only JSONL 会归类为 provider failure，并带结构化 diagnostics，successful search 写 report | 产品功能/外部研究 | `src/isotope/features/research/providers.py` |
...
| `research.recall` | capability runner 里的 Research artifact preview 召回能力，扫描现有 `research.report` 顶层 metadata，复用 `rag.index` / hybrid retrieval，只返回 summary、ref、source_refs 和 provenance；report 正文仍走 artifact inspect/expand | 产品能力/外部研究/RAG | `src/isotope/capabilities/research.py`, `src/isotope/features/research/recall.py`, `src/isotope/rag/index.py` |
| `research.promote` | capability runner 里的 Research 记忆提案能力，复用 existing research promote payload builder 和 `memory.promotion` 边界，从 `research.report` metadata 生成 proposal summary，memory 写入走 approval | 产品能力/外部研究/记忆 | `src/isotope/capabilities/research.py`, `src/isotope/features/research/runner.py`, `src/isotope/memory/promotion.py` |
...
| `screen_control` | Screen 控制 tool，默认 dry-run 生成控制计划；execute 需要显式批准或策略放行；测试/手动 smoke 可用 `approval_mode=yolo` 临时放行，但必须同时提供显式 `target_allowlist`；多窗口匹配时默认拒绝 first-match 真实输入；`restore_window` 也是受同一 approval gate 保护的控制动作 | 工具/本机控制 | `src/isotope/execution/screen/backend_adapter.py`, `src/isotope/policy/__init__.py` |
...
| `screen.control` | capability runner 里的 Screen 控制能力，复用 `screen_control` tool、target selector、dry-run plan 和 execute approval gate；桌面审批只展示 action count/type、selector keys 和 execution mode 等低敏摘要，不暴露坐标或按键 payload | 产品能力/本机控制 | `src/isotope/capabilities/screen.py`, `src/isotope/runtime/in_process/actions.py`, `src/isotope/features/supervisor/desktop_snapshot.py`, `apps/desktop/src/lib/components/main/ConversationWorkspace.svelte` |
| `isotope-screen` | Screen 手动 smoke 命令行入口，用于本机窗口 observe/control 验收；`control-click` 可生成 click dry-run plan，`control-restore` 可生成恢复窗口 dry-run plan 或在显式批准后执行，`--approval-mode yolo` 可在带 target allowlist 的测试命令中直接执行，`real-smoke-plan` 输出真实 backend 验证命令，`inspect/report` 可读取 screen artifact 或生成结构化 observe/control plan 摘要，`--allow-app` / `--allow-title-contains` 可注入命令级 target allowlist，`--allowlist-file` 可读取可复用 JSON target allowlist，`--allowlist-profile` 可从 profile 目录解析命名 allowlist，`allowlist validate` 可按 path 或 profile 离线检查 allowlist 格式并输出结构化计数，`allowlist list --profile-dir` 可列出 profile 名和结构化计数，`allowlist template` 可打印可编辑 JSON 骨架；不属于默认 CI | 应用入口/本机控制 | `src/isotope/features/screen/runner.py`, `src/isotope/features/screen/artifacts.py` |
...
| `current batch` | 当前批次视图，把仍活跃目标和当前托管 worker 从历史 done/stale session 中分离 | 产品功能/视图/状态判断 | `src/isotope/agents/scheduler/current_batch.py`, `src/isotope/features/supervisor/current_batch.py`, `src/isotope/features/supervisor/web.py` |
...
| `/llm-action` | web 手动模型建议入口，展示 LLM planner 的受控动作建议；发送动作走显式执行入口 | 产品功能/模型/控制策略 | `src/isotope/features/supervisor/web.py` |
...
| `supervisor_action` | Supervisor 对外统一动作出口；LLM planner 读取程序预查事实和候选后选择最终白名单动作；旧 `llm_action` 只是兼容镜像 | 产品功能/控制策略/输出字段 | `src/isotope/features/supervisor/commands/supervisor_action.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
| `supervisor_prepared_action_context` | `loop --json` / LLM prompt 中的程序预查上下文，包含候选动作 `candidates` 和已整理事实 `facts`；事实覆盖 active goals、decision requests、context history、worker reviews、清理候选和 capacity decisions，减少模型重复工具查询，但不是最终动作决定 | 产品功能/模型上下文/输出字段 | `src/isotope/features/supervisor/commands/supervise/program_action.py`, `src/isotope/features/supervisor/llm_action/prompt.py` |
| `supervisor_action_planner` | `loop --json` / dashboard payload 中的动作来源说明；`source=llm` 表示最终 `supervisor_action` 由 LLM planner 选择，`reason=prepared_context` 表示模型使用了程序预查上下文 | 产品功能/控制策略/输出字段 | `src/isotope/features/supervisor/commands/supervisor_action.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
| `LLM planner` | 大模型规划器，读取程序预查事实和候选，选择 `monitor`、send、`resume_session`、`launch_session`、`request_context`、`ask_user`、`delete_worktree` 或 `call_capacity` | 产品功能/模型/控制策略 | `src/isotope/features/supervisor/llm_action/llm_summary.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
| `LLM action` | 旧命名下的大模型规划结果；公共 payload 优先看 `supervisor_action`，`llm_action` 保留为兼容镜像 | 产品功能/模型/兼容字段 | `src/isotope/features/supervisor/commands/supervisor_action.py`, `src/isotope/features/supervisor/llm_action/llm_summary.py` |
...
| `--llm-action` | 命令行参数，让 LLM planner 读取最近 context 结果并选择一个受控建议动作；执行走 `--llm-execute` | 产品功能/模型/控制策略 | `src/isotope/features/supervisor/runner.py` |
...
| `cleanup list/archive/delete-worktree` | Supervisor 生命周期清理命令，列出或归档 done goal、done managed worker 和 done 通知；`delete-worktree` 删除已归档且已集成的 Supervisor worktree，Codex 历史和 git branch 由各自命令管理；loop 可自动清理本轮刚归档的 source/merge worktree | 产品功能/控制通道/状态账本 | `src/isotope/features/supervisor/runner.py` |
...
| `loop` | Supervisor 日常常驻入口，默认先由程序收集状态和候选动作，再让 LLM planner 选择最终 `supervisor_action` | 产品功能/控制通道 | `src/isotope/features/supervisor/supervise/payload.py`, `src/isotope/features/supervisor/commands/supervise/action.py` |
...
| `--webhook-secret` | Supervisor webhook 共享密钥；配置后用请求 body 生成 `X-Isotope-Signature: sha256=...` HMAC 签名，payload 保持事件字段 | 产品功能/通知/安全 | `src/isotope/features/supervisor/runner.py`, `src/isotope/features/supervisor/notifications.py` |
...
| `integration-review` | 集成审查命令，扫描 managed worker 的 branch、worker HEAD、base ref 包含关系、pytest gate、worktree 干净状态、merge conflict 风险和候选 lint/test 结果；输出分组和理由，merge、push、删除交给 merge dispatch / worker 继续处理 | 产品功能/状态判断/集成审查 | `src/isotope/features/supervisor/integration_review.py`, `src/isotope/features/supervisor/runner.py` |
...
| `merge dispatch` | 合并派发层，在 `loop` 中读取 `integration-review` 的 `ready_to_integrate` 候选，生成 `merge-work-order`，再复用 `launch_session` 路径启动专门 merge worker；cherry-pick、branch 删除和历史改写由 merge worker / git 命令显式处理 | 产品功能/控制策略/控制通道 | `src/isotope/features/supervisor/merge_dispatch.py`, `src/isotope/features/supervisor/runner.py` |
...
| `decision request` | 拍板请求账本项，记录合法 `ask_user` 的问题、session 或 `goal_id`、原因和 gate 证据，供 dashboard / web 展示，也会在 LLM planner 决策前进入 `supervisor_prepared_action_context.facts`；有答案用 `decision answer`，无需继续才归档 | 产品功能/通知/拍板/模型输入 | `src/isotope/features/supervisor/decision_requests.py`, `src/isotope/features/supervisor/runner.py`, `src/isotope/features/supervisor/web.py` |
...
| `decision archive` | 拍板归档事件，追加写入 JSONL，让无需继续的拍板项从活跃列表移走，历史保留在事件账本 | 产品功能/通知/拍板 | `src/isotope/features/supervisor/decision_requests.py`, `src/isotope/features/supervisor/runner.py` |
...
| `capacity calling` | 能力调用，让 LLM 在候选能力中选择一个能力并填参数，系统再按能力契约执行 | 模型/能力/智能体 | `src/isotope/llm/capacity_calling.py`, `src/isotope/agents/loop/step.py`, `src/isotope/capabilities/runner.py` |
...
| `capacity plan` | Supervisor 能力调用入口，LLM 选择能力并填参数，系统生成依赖图和 launch plan；输入齐备且执行路径开启时进入 agent loop 调用 allowlist 能力，缺输入返回 `request_input` | 产品功能/模型/能力 | `src/isotope/features/supervisor/commands/handlers/capacity.py`, `isotope-supervisor capacity plan` |
...
| `provider planner tick` | provider 驱动的单 tick 规划入口，用注入的 LLM provider 生成 JSON planner decision，再走现有 planner contract 和 step driver；当前测试使用 deterministic test provider，真实网络由 provider 配置负责；prompt 默认带 `default_context.memory` 结构化记忆 preview | 智能体/模型 | `src/isotope/agents/loop/provider_planner.py`, `src/isotope/agents/loop/context.py` |
...
| `hybrid retrieval` | 通用 RAG 检索基建，先用 BM25 稀疏检索，配置 dense backend 时再融合向量检索；`rag.index` 负责 local deterministic dense 索引和可选 LanceDB vector store 索引装配，`memory.query` / `memory.recall` 和 `research.recall` 是第一批 caller，未配置、缺依赖或写查失败时降级 BM25；LanceDB backend 通过 `.[rag]` 安装，`.[test]` 包含真实 LanceDB smoke | RAG/检索 | `src/isotope/rag/hybrid.py`, `src/isotope/rag/index.py`, `src/isotope/rag/lancedb_store.py` |
...
| `memory.query` | capability runner 里的 memory recall 能力，复用 `LocalMemoryQueryService` 和 caller audit，只返回 summary / refs / provenance；默认 BM25，可用 `dense_retrieval={"backend":"local"}` 显式启用本地 dense smoke，也可加 `embedding_provider="fastembed"` 接真实本地 embedding，或用 `dense_retrieval={"backend":"lancedb","path":"...","table_name":"..."}` 接入可选 LanceDB vector store | 产品能力/智能体 | `src/isotope/capabilities/memory.py`, `src/isotope/memory/__init__.py` |
