...
| `isotope-supervisor` | Codex Supervisor 命令行入口，支持扫描、建议面板、定时汇报、变化触发、托管启动和发送指令 | 应用入口 | `src/isotope/features/supervisor/runner.py`, `apps/cli/isotope_supervisor.py` |
...
| `advise` | 建议面板命令，只输出当前建议和命令草案，不执行命令 | 产品功能/控制策略 | `src/isotope/features/supervisor/runner.py` |
