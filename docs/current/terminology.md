...
| `worker_lifecycle_execution` | `loop --json` / dashboard 输出字段，记录程序从 `worker_lifecycle_decision.next_step` 归一出的固定流程执行计划；dashboard 会给需要显式确认的计划展示 one-shot `execute_command` 复制入口，并通过 `/worker-lifecycle/execute` 校验当前计划后执行固定 loop 命令；执行完成后用低敏 `result_actions` / `result_summary` 展示动作结果 | 产品功能/输出字段/控制策略 | `src/isotope/features/supervisor/lifecycle/executor.py`, `src/isotope/features/supervisor/supervise/payload.py`, `src/isotope/features/supervisor/dashboard/_presentation.py`, `src/isotope/features/supervisor/web/routes/worker_lifecycle.py` |
