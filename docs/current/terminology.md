...
| `hybrid retrieval` | 通用 RAG 检索基建，先用 BM25 稀疏检索，配置 dense backend 时再融合向量检索；`rag.index` 负责本地 deterministic dense 索引装配，`memory.query` / `memory.recall` 是第一批 caller，第一外部后端目标仍是 LanceDB，未配置或失败时降级 BM25 | RAG/检索 | `src/isotope/rag/hybrid.py`, `src/isotope/rag/index.py`, `src/isotope/rag/lancedb_store.py` |
