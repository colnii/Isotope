...
| `hybrid retrieval` | 通用 RAG 检索基建，先用 BM25 稀疏检索，配置 dense backend 时再融合向量检索；`memory.query` / `memory.recall` 可显式启用本地 deterministic dense 闭环，第一外部后端目标仍是 LanceDB，未配置或失败时降级 BM25 | RAG/检索 | `src/isotope/rag/hybrid.py`, `src/isotope/memory/dense.py`, `src/isotope/rag/lancedb_store.py` |
