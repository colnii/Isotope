...
| `supervisor_prepared_action_context` | `loop --json` / LLM prompt 中的程序预查上下文，当前包含 lifecycle / merge 候选动作和理由；它减少模型重复工具查询，但不是最终动作决定 | 产品功能/模型上下文/输出字段 | `src/isotope/features/supervisor/commands/supervise/program_action.py`, `src/isotope/features/supervisor/llm_action/prompt.py` |
