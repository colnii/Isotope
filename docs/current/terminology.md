...
| `artifact` | 产物记录，保存执行结果摘要和引用 | 核心数据 | `src/isotope/models.py` |
| `ResourceRef` | 资源引用，指向产物等对象而不是直接暴露全文 | 核心数据 | `src/isotope/refs.py` |
...
| `RunState` | 运行状态，投影后的当前视图 | 状态恢复 | `src/isotope/models.py` |
| `policy` | 权限策略，决定动作是否允许、暂停或拒绝 | 安全/权限 | `src/isotope/policy.py` |
| `approval` | 人工确认，敏感动作执行前的暂停和恢复机制 | 权限/产品 | `src/isotope/server.py` |
| `capability` | 能力，产品可发现、可运行的功能单元 | 产品能力 | `src/isotope/capability_catalog.py` |
| `capability runner` | 能力运行器，用命令行方式搜索能力、生成计划或启动能力 | 产品能力 | `src/isotope/capability_runner.py` |
