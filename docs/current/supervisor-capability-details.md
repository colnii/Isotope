...
| 状态投影层 | `build_supervisor_state_snapshot(...)` | `features/supervisor/state/projection.py`、`platform/state/supervisor_snapshot.py`、`platform/state/goal_status.py` | 只读聚合 active goals、decision requests、lane failure、worker events 和 notifications；dashboard/web/daemon 已读取，loop payload 已带只读 snapshot；输出结构复用 `SupervisorStateSnapshot` 和 `SupervisorGoalStatus` schema |
...
1. 继续评估 lane state、goal queue 持久化和 notification index 中哪些事实应下沉到
