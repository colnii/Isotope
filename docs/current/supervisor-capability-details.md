...
- 主路径已经接入：Web/CLI、goal queue、fanout、current batch、
  ...
  capability、`supervisor.integration_review` capability、Supervisor state
  projection（状态投影）。
...
| Worker 审查层 | `worker-review`、`supervisor.worker_review`、`integration-review`、`supervisor.integration_review`、`replan` | `features/supervisor/worker_review.py`、`features/supervisor/integration_review.py`、`capabilities/runner.py`、`features/supervisor/replan.py`、`features/supervisor/commands/handlers/replan.py` | 汇总已托管 worker 的 worktree、branch、状态协议、改动、复查提示、合并提示、只读集成分组和下一轮候选；capability runner 已提供 lightweight 只读 worker-review 和 integration-review 包装 |
| Screen 报告层 | `isotope-screen report`、`isotope-supervisor screen report`、`screen.report` | `features/screen/artifacts.py`、`features/supervisor/commands/dispatch.py`、`capabilities/screen.py`、`capabilities/runner.py` | 汇总已有 screen observe/control artifact，返回 target metadata、截图可用性、恢复建议和 control plan approval/interference 标记；capability runner 只读复用同一报告边界，不读取截图正文、不执行输入、不改变窗口 |
...
- `capabilities/memory.py`：已承接 `memory.query` capability runner wrapper；
  ...
  provenance；`controlled_expand` 有 expand grant 和正预算时会物化 matched
  `MemoryRecord.content` 的 budgeted `materialized_text`，不读取 source artifact
  full content。
