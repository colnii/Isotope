...
| 状态投影层 | `build_supervisor_state_snapshot(...)` | `features/supervisor/state/projection.py` | 只读聚合 active goals、decision requests、lane failure、worker events 和 notifications；dashboard/web/daemon 已读取，后续继续给 loop 等入口统一复用 |
...
1. 让 loop 和其他只读状态入口继续复用 Supervisor state projection。
