...
本文从原 `supervisor-capability-map.md` 拆出，保留详细能力登记、分层说明和
后续拆分方向。短索引见
...
| 状态投影层 | `build_supervisor_state_snapshot(...)` | `features/supervisor/state/projection.py`、`platform/state/supervisor_snapshot.py`、`platform/state/goal_status.py`、`platform/state/lane_state.py` | 只读聚合 active goals、decision requests、lane failure、worker events 和 notifications；dashboard/web/daemon 已读取，loop payload 已带只读 snapshot；输出结构复用 `SupervisorStateSnapshot`、`SupervisorGoalStatus` 和 `SupervisorLaneState` schema |
...
- `web` 顶部会显示 `state_snapshot_meta.schema_label`，让页面上直接可见
  当前读取的 Supervisor state snapshot 版本。
...
- `capacity calling` 当前会做两层低敏 contract guardrail（契约护栏）：
  ...
  输入名，以及 `required` 未在 `properties` 声明的坏 contract；provider
...
- `features/supervisor/goal_queue.py`：仍负责 `goals.jsonl` 写入、去重和
  goal status 通知；最近状态 payload 已改为复用
  `platform/state/goal_status.py` 的 `SupervisorGoalStatus`，当前不迁
  goal queue 持久化格式。
...
1. 继续评估 lane state 持久化、goal queue 持久化和 notification index 中哪些事实应下沉到
