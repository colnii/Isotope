...
本文从 [Supervisor 能力详情](./supervisor-capability-details.md) 拆出，
保留现有能力、已复用 helper 和 guardrail（护栏）的长清单。拆分原因是
这些条目会随 main 上的小重构频繁更新，独立成文档可以降低
`supervisor-capability-details.md` 的 rebase 冲突面。

短索引见 [Supervisor 能力地图](./supervisor-capability-map.md)。架构归属和迁移判断见
...
- `dashboard JSON` 和 `/dashboard.json` 会在顶层输出
  ...
  `state_snapshot` 也能确认读模型版本；缺少 `kind` 或 `schema_version`
  时会标成 `degraded`，避免把旧形状误读成正常读模型。
...
- `capacity calling` 当前会做两层低敏 contract guardrail（契约护栏）：
  ...
  `CapabilityRunner` 复用，避免两条路径各自处理 malformed contract；provider
...
- `memory.query` 已注册为可发现、可预检、可运行的只读 capability：
...
- `memory.promotion.preview` 已注册为可发现、可预检、可运行的只读
  ...
  `run_id/agent_id/thread_id/candidate`，可选 `scope/quality`；执行时复用
  `memory.promotion` proposal boundary，只从 structured artifact metadata 或
...
- `--llm-execute` 执行合法 `ask_user` 时会写入
  ...
  稳定拍板列表；state projection 的 active decision payload 已改为复用
  `platform/state/decision_request.py` 的 `SupervisorDecisionRequest`，但
  ledger 写入仍复用 `DecisionRequestLedger`；同一 `session_id`
  和问题已有活跃请求时会复用旧项，不重复追加和通知。
