...
| state projection（状态投影） | `features/supervisor/state/projection.py` | 先留在 `features/supervisor/state/`，后续按事实层下沉 | 已新增只读 snapshot 聚合 active goals、decision requests、lane failure、worker events 和 notifications；dashboard/web 已读取，下一步接 daemon | 当前只做 read model，不新增账本、不改写入格式。 |
