...
| `capabilities/` | `CapabilityRunner` 可列出、搜索、规划和受控运行少量 allowlist 能力；Supervisor `capacity plan` 已复用 launch plan；runner `plan/run` 和 LLM capacity calling 共用 `platform/schemas/input_contract.py` 校验 contract keys、type 和 enum | 能力目录目前很薄，真实 Supervisor worker/merge/context 能力尚未注册进 catalog | 把可复用操作登记成能力，由 LLM 选择能力，再由 runner/loop 执行 |
