...
| `capabilities/` | `CapabilityRunner` 可列出、搜索、规划和受控运行少量 allowlist 能力；Supervisor `capacity plan` 已复用 launch plan | 能力目录目前很薄，真实 Supervisor worker/merge/context 能力尚未注册进 catalog | 把可复用操作登记成能力，由 LLM 选择能力，再由 runner/loop 执行 |
...
| `platform/state/` | `DecisionRequestLedger`、`FailureLedger`、`FileMemoryStore`、worker event channel 和 multi-worker read model 已被 Supervisor adapter 复用；event/memory/checkpoint/projector 已服务 runtime | lane state、goal status、notification index 仍多为 feature 私有 JSONL | 继续统一跨 worker 事实、事件、记忆和拍板账本，dashboard 读取投影而不是拼散表 |
...
| merge dispatch / promotion | `merge_dispatch.py`, `commands/merge_dispatch.py`, `merge_promotion.py`, `runner.py` | `workspace/git/` + `agents/integration/` | merge dispatch loop 编排已迁出 runner；下一步拆 promotion gate、CI watch 和 git/worktree 操作 | 合并、CI、worktree 清理属于 workspace/integration。 |
...
| state projection（状态投影） | `features/supervisor/state/projection.py` | 先留在 `features/supervisor/state/`，后续按事实层下沉 | 已新增只读 snapshot 聚合 active goals、decision requests、lane failure、worker events 和 notifications；dashboard/web/daemon 已读取，loop payload 已带只读 snapshot | 当前只做 read model，不新增账本、不改写入格式。 |
