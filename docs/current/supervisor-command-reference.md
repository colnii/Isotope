...
| `isotope-capability` | 搜索、预检或运行低敏 capability；`supervisor.worker_review` / `supervisor.integration_review` 复用既有只读审查路径，`memory.query` / `screen.report` 复用既有低敏查询/报告边界。 | [capability inventory](./supervisor-capability-inventory.md) |
