...
- 如果 `codex_task` 仍在 `deferred_tools`，fail closed 为 `IsotopeError(code="model_tool_not_enabled")`，不追加 events / artifacts。
...
- 如果模型选择当前 bridge 没有 route 的其他 enabled tool，fail closed 为 `IsotopeError(code="model_tool_route_not_enabled")`。
...
- route 返回的 structured error 会保留为 `IsotopeError`，例如 unknown run 仍是 `not_found`，而不是退化成普通异常。
...
下一步若继续，应从 `llm-provider-tool-call-boundary-v0.2.md` 的 reopen conditions 走；不要让模型绕过 approval。
