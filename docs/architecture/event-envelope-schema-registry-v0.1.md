...
它不是 event payload schema registry，不解释每个 `event_type` 的业务 payload，也不生成 `RunState`。Event payload schema / compatibility 的后续 v0.2 边界见 `event-schema-registry-compatibility-boundary-v0.2.md`。
...
- Event payload schema registry / compatibility first slice 已独立实现并 closed for now：known canonical event type metadata、unknown event fail-closed、unsupported payload schema version fail-closed；它不改变本 envelope registry 边界。Closure review 见 `event-schema-registry-closure-review.md`。
