...
Option A, a third app spike, should wait until after Kernel Gap Review Refresh unless the user explicitly wants more usability pressure now. That refresh has now landed in `kernel-gap-review-refresh-v0.2.md` and recommends `Workspace Resource Lifecycle Boundary` next.
...
- `kernel-gap-review-refresh-v0.2.md` now records first-slice enough surfaces and still-open kernel-level gaps。
- Follow-up boundary is `../architecture/workspace-resource-lifecycle-boundary-v0.2.md`。
