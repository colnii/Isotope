...
- Capability Runner Thin Shell Boundary 已落地，见 `docs/capability-runner-thin-shell-boundary-v0.2.md`。该 boundary 明确下一步只做基于现有 `capability_catalog` 的薄执行壳：`list / describe / status / run`，第一批只允许 `artifact.review`、`external.snapshot.review`、`approval.tool.runner` 等现有 product-candidate capability deterministic run；不复制 aggressive `capability_hub.py` / `self_evolution.py`，不打开 workflow engine、product shell、study companion、自我进化 harness 或 real provider runner。
