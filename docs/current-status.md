...
- 最新 docs/status target：Tool Protocol Boundary；当前已经 external review ready，见 `docs/post-external-review-checkpoint.md`、`docs/kernel-mainline-maintenance-mode.md` 和 `docs/mainline-idle-checkpoint.md`。外部 reviewer 入口是 `docs/external-review-package-v0.2.md`，它用一个短路径说明 Isotope 现在是 event-sourced agent kernel prototype、能跑哪些 demos、这些 demos 证明了哪些 kernel contracts、以及哪些 product / integration surfaces 仍 deliberately not implemented。主线当前仍处于 idle / conservative maintenance mode；本轮只是用户显式要求的 docs-only `docs/tool-protocol-boundary-v0.2.md` boundary，不代表主动恢复 kernel expansion。application-layer prototype / aggressive branch 仍应先制造真实 friction，kernel mainline 只接收应用层或外部 review 已证明必要的 helper / boundary / replay / checkpoint / API ergonomics gap。
...
- Tool Protocol Boundary docs-only defined
...
- tool failure should become controlled `action.failed` / structured error without half-written artifact
