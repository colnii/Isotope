...
- Error Taxonomy Boundary 已落文档，见 `docs/error-taxonomy-boundary-v0.2.md`。这是 docs-only planning，来自 aggressive-dev / review 接受的 `unstructured_kernel_helper_errors` pressure：HTTP facade 已有 structured error envelope，但 direct helpers 仍多为 plain `ValueError(...)`。边界建议后续 red tests 固定 `KernelError(ValueError)` compatibility shape：保留 `str(exc)` / `args[0]` message，同时暴露 stable `code`、`category`、`retryable`、optional `http_status` 和 low-sensitive `details`；不做 product error UX、public SDK、real HTTP server、provider/process/container/git-worktree errors、tag 或 release。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Post External Review Checkpoint、Kernel Mainline Maintenance Check、Mainline Idle Checkpoint 和 Session / Run Lifecycle first slice 当前 complete；Next Suggested Mode 是 application-layer friction intake、external review feedback intake，或由 aggressive-dev 提供下一个 concrete `kernel_friction`。
