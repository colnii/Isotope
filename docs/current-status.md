...
- 最新 docs/status target：Policy Profile / Action Registry Versioning Boundary；本轮只定义 docs-only kernel contract，不实现 plugin / marketplace / policy DSL / migration framework。
...
- v0.1 developer demo 已按 `docs/demo/v0.1-demo-acceptance.md` accepted：acceptance anchor 当时依据是 `568 passed`、demo plain / JSON 本地可运行、editable install smoke 已覆盖、远端 GitHub Actions CI 已由网页确认通过；当前 baseline 已随 workspace resource lifecycle slice 更新为 `942 passed`。
...
- Kernel Gap Review Refresh 已落文档，见 `docs/kernel-gap-review-refresh-v0.2.md`。当前判断：agent / worker lifecycle、workspace substrate、workspace resource lifecycle、retry / cancel / supersede、HTTP facade、approval pause / resume、external ingestion boundary 和 artifact content read policy 都已足够 first-slice；仍 open 的 kernel-level gaps 是 policy profile / action registry versioning、retry / cancel / supersede runtime integration、worker handoff app composition、session / run lifecycle、error taxonomy、event schema registry、tool protocol 和 future real workspace substrate。Policy Profile / Action Registry Versioning Boundary 已定义，见 `docs/policy-profile-action-registry-versioning-boundary-v0.2.md`；下一步推荐 red tests only，不要直接开 real HTTP server、real LLM、provider adapter、memory query engine、filesystem sandbox / container 或第三个 app spike。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Policy Profile / Action Registry Versioning Boundary 当前 complete；Next Suggested Batch 建议 `Policy Profile / Action Registry Versioning Red Tests`。
...
- Policy Profile / Action Registry Versioning Boundary 已落文档
- `docs/policy-profile-action-registry-versioning-boundary-v0.2.md`
- `ActionProposal` / `action.proposed` 后续应记录 registry/version basis
- `PolicyDecision` / `action.decided` 后续应记录 policy profile/version basis
...
- reason codes should become stable identifiers, not free-form prose
- first red tests recommendation: `tests/isotope_kernel/test_action_registry_version_basis.py` and `tests/isotope_kernel/test_policy_profile_version_basis.py`
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `942 passed` after workspace resource lifecycle green slice
...
- v0.2 acceptance evidence includes `735 passed` at the tag anchor, v0.1 demo plain / JSON success, v0.2 demo plain / JSON success, configured CI smoke, no `x_agent.*` imports, and `/home/lumber/Github/x-agent` untouched; current mainline baseline is `942 passed`
...
- follow `docs/agent-task-queue.md` rolling batch mode；Policy Profile / Action Registry Versioning Boundary 已完成。下一步推荐 `Policy Profile / Action Registry Versioning Red Tests`；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine 或 filesystem/container substrate。
