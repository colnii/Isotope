...
- v0.1 developer demo 已按 `docs/demo/v0.1-demo-acceptance.md` accepted：acceptance anchor 当时依据是 `568 passed`、demo plain / JSON 本地可运行、editable install smoke 已覆盖、远端 GitHub Actions CI 已由网页确认通过；当前 baseline 已随 Agent / Worker lifecycle first slice 更新为 `786 passed`。
...
- Workspace Substrate Boundary v0.2 已落文档，见 `docs/workspace-substrate-boundary-v0.2.md`。当前只定义 boundary：workspace 是 policy-bound execution resource，不是 agent identity；binding 必须来自 grants；write / isolated mode 不能隐式升级；artifact capture 必须走 artifact / provenance；projector 不能读取 workspace files 推进 native state。当前实现仍只是 `WorkspaceManager` shared read-only / grants validation，未实现 container、git worktree、remote executor、rollback 或 cleanup scheduler。
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `786 passed` after the Agent / Worker lifecycle first slice
...
- Workspace Substrate Boundary v0.2 已定义；下一步如继续 implementation，应先写 workspace binding read model / policy boundary red tests
