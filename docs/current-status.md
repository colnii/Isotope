...
- Capability Runner Thin Shell Closure Review 已落地，见 `docs/capability-runner-thin-shell-closure-review.md`。后续用户明确要求继续消化 aggressive 剩余代码后，主线已抽取最小 CLI first slice，见 `docs/capability-runner-cli-boundary-v0.2.md`：`python -m isotope_kernel.capability_runner` 支持低敏 `list / describe / status / run`，仍只调用现有 thin runner allowlist，不复制 aggressive `capability_hub.py`，不实现 `ask` / `interactive` / workflow engine / product shell / real provider runner。CLI 后的 intake refresh 见 `docs/aggressive-remaining-code-intake-refresh-v0.md`：aggressive 剩余代码仍只能 extract-only；下一步推荐 `Capability Search / Launch Plan Boundary`，先做 search / preflight / launch-plan contract，不直接做 provider-backed runner 或 self-evolution。
