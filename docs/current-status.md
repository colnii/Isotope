...
- 最新 implementation slice target：external snapshot review demo；当前 docs/status sync target 是 External Snapshot Review Green Slice。
...
- Second App Spike Selection 已落文档，见 `docs/second-app-spike-selection.md`。第二个 app spike 已选择并实现为 `external snapshot review` first slice：`python -m isotope_kernel.demo --scenario external-snapshot-review`、`--trace` 和 `--json` 已可运行，展示 Track F / `ImportedSnapshot` / `RunState.external_observations` / conflict diagnostics / native state priority / replay / checkpoint，同时仍保持 deterministic / in-process / no provider adapter / no webhook / no real HTTP server / no real LLM / no filesystem mutation / no memory query engine。HTTP `/external-ingestion` 仍 `501 not_enabled`。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。External Snapshot Review Green Slice 当前 complete；Next Suggested Batch 是 `External Snapshot Review Friction Review`，docs-only by default。
...
- external snapshot review second app spike first slice
...
- follow `docs/agent-task-queue.md` rolling batch mode；External Snapshot Review Green Slice 已完成。下一步推荐 `External Snapshot Review Friction Review`，docs-only by default；不要直接扩大 Track F provider / webhook / HTTP ingestion API。
