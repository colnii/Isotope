...
- Worker Handoff App Spike Selection 已落文档，见 `docs/worker-handoff-app-spike-selection.md`。结论：下一步可做 red-tests-only `Worker Handoff App Spike` pressure test，用 deterministic in-process scenario 压测 worker lifecycle / delegation policy / workspace grants / artifact `ResourceRef` handoff / replay / checkpoint；仍不得打开 real concurrency、process spawn、remote worker、container、git worktree、real HTTP、LLM、provider、public SDK 或 product multi-agent UX。
