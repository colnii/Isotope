...
- Post `v0.2-demo` tag delta 已记录在 `docs/post-v0.2-tag-delta.md`。当前 `main` ahead of `v0.2-demo` 的主要增量是 Track F external ingestion boundary / read-model / checkpoint support、Agent / Worker lifecycle first slice 和 Workspace substrate first slice。默认不移动 `v0.2-demo`，也暂不创建 `v0.2.1-demo`；只有外部 reviewer 需要固定包含 Track F / Agent Worker / Workspace slices 的锚点时再准备新 tag。
