...
- Session / Run Lifecycle Boundary 已落文档，见 `docs/session-run-lifecycle-boundary-v0.2.md`。这是 docs-only planning：当前明确 session 仍主要是 in-process server metadata，run lifecycle 已有 `run.created` / `run.completed` 和 `RunState.status`，但缺统一 session event/read-model、terminal transition 和 checkpoint/read helper contract；下一步只建议 red tests，不实现 product session workflow、real HTTP/auth/UI、scheduler/process kill 或 run graph。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Post External Review Checkpoint、Kernel Mainline Maintenance Check 和 Mainline Idle Checkpoint 当前 complete；Next Suggested Mode 是 application-layer friction intake、external review feedback intake 或 periodic verification only，不再主动扩 kernel feature。
...
- follow `docs/agent-task-queue.md` rolling batch mode；External Review Package 和 Post External Review Checkpoint 已刷新，见 `docs/external-review-package-v0.2.md` 和 `docs/post-external-review-checkpoint.md`。默认建议短暂停止 kernel expansion，让 application-layer prototype 先制造真实 friction；Tool Protocol first slice 已完成 closure review，下一步回到 application-layer friction intake / external review feedback intake。
