...
- 最新 docs/status target：Event Schema Registry / Compatibility Boundary；`docs/event-schema-registry-compatibility-boundary-v0.2.md` 已定义 event payload schema registry / compatibility 的最小 contract，下一步是 red tests only。当前仍不实现 schema migration framework、JSON Schema / protobuf dependency、plugin event types 或 remote registry。
...
- v0.1 developer demo 已按 `docs/demo/v0.1-demo-acceptance.md` accepted：acceptance anchor 当时依据是 `568 passed`、demo plain / JSON 本地可运行、editable install smoke 已覆盖、远端 GitHub Actions CI 已由网页确认通过；当前 baseline 已随 Retry / Cancel / Supersede runtime helper slice 更新为 `974 passed`。
...
- Kernel Gap Review Refresh 已落文档，见 `docs/kernel-gap-review-refresh-v0.2.md`。当前判断：agent / worker lifecycle、workspace substrate、workspace resource lifecycle、retry / cancel / supersede、HTTP facade、approval pause / resume、external ingestion boundary、artifact content read policy 和 policy profile / action registry versioning 都已足够 first-slice；仍 open 的 kernel-level gaps 是 worker handoff app composition、session / run lifecycle、error taxonomy、event schema registry、tool protocol 和 future real workspace substrate。Policy Profile / Action Registry Versioning first slice 已完成 closure review，见 `docs/policy-registry-version-basis-closure-review.md`；Retry / Cancel / Supersede Runtime Integration first slice 已完成 closure review，见 `docs/retry-cancel-supersede-runtime-closure-review.md`。Event Schema Registry / Compatibility Boundary docs-only slice 已完成，见 `docs/event-schema-registry-compatibility-boundary-v0.2.md`；下一步推荐 red tests only，不要直接开 real HTTP server、real LLM、provider adapter、memory query engine、filesystem sandbox / container、plugin marketplace、policy DSL、schema migration framework、scheduler、process kill 或第三个 app spike。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Event Schema Registry / Compatibility Boundary 当前 complete；Next Suggested Batch 建议 `Event Schema Registry / Compatibility Red Tests`。
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `974 passed` after Retry / Cancel / Supersede runtime helper green slice
...
- v0.2 acceptance evidence includes `735 passed` at the tag anchor, v0.1 demo plain / JSON success, v0.2 demo plain / JSON success, configured CI smoke, no `x_agent.*` imports, and `/home/lumber/Github/x-agent` untouched; current mainline baseline is `974 passed`
...
- follow `docs/agent-task-queue.md` rolling batch mode；Event Schema Registry / Compatibility Boundary docs-only slice 已 complete。下一步推荐 `Event Schema Registry / Compatibility Red Tests`；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine、plugin marketplace、policy DSL、schema migration framework、scheduler、process kill 或 filesystem/container substrate。
