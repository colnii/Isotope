...
- Mainline 已根据 aggressive branch `1950e32` / review feedback 重新打开并完成一个 bounded kernel helper slice：`private_append_worker_handoff` 被确认是真实 `kernel_friction`，当前由 `InProcessServer.submit_worker_handoff(...)` first green slice 解决。边界文档见 `docs/worker-handoff-helper-boundary-v0.2.md`；helper 只做 deterministic in-process canonical delegation / worker result handoff，不实现 real concurrency / process spawn / remote worker / container / git worktree / real HTTP / LLM / provider / public SDK。
