...
- Mainline 已根据 aggressive branch `1950e32` / review feedback 重新打开一个 bounded kernel slice：`private_append_worker_handoff` 被确认是真实 `kernel_friction`。边界文档见 `docs/worker-handoff-helper-boundary-v0.2.md`；下一步只建议先写 `tests/isotope_kernel/test_worker_handoff_helper.py` red tests，不实现 real concurrency / process spawn / remote worker / container / git worktree / real HTTP / LLM / provider / public SDK。
