...
- 最新 implementation commit：`fc72cdac0159fd4ed404391ad7b7b06f792e3077`。
...
- v0.1 developer demo 已按 `docs/v0.1-demo-acceptance.md` accepted：acceptance anchor 当时依据是 `568 passed`、demo plain / JSON 本地可运行、editable install smoke 已覆盖、远端 GitHub Actions CI 已由网页确认通过；当前 baseline 已随 Track A validation slice 更新为 `606 passed`。
...
- v0.2 roadmap 已开始，见 `docs/v0.2-roadmap.md`。Track D: Demo / Docs Polish 当前已 effectively complete / closed for now；Track A: HTTP API Minimal Surface 已完成 minimal surface 和 request validation / no-side-effect error boundary 两批 green slice。
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `606 passed` after the Track A HTTP API request validation slice
...
- Track A has landed two in-process HTTP API slices: minimal surface plus request validation / no-side-effect error boundary; no real listening server, web framework, auth, SSE, memory query API, external ingestion API, or full artifact content API is implemented
