...
- Planner I/O Validator Spike 已落地，见 `docs/planner-io-validator-spike-review.md`。`python -m isotope_kernel.demo --scenario agent-loop-planner-io-validator` / `--trace` / `--json` 验证一个 demo-local “门卫”：valid symbolic planner output 可通过，malformed / unknown action / overpowered capability / unauthorized artifact full-text request 会在执行前 fail closed。当前结果是 `planner_io_validator_ok=true`、`partial_events_appended=false`、`kernel_friction=[]`。下一步建议是 `Planner Validated Runner Spike`：把门卫接到小 runner 上，但仍不接真实模型。
