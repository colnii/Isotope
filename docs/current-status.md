...
- Controlled terminal / provider integration 已合入 `main`：`feature/controlled-terminal-exec` 的受控 terminal、Codex task route、model-tool bridge、LLM provider route、tool-result loop、product-chat route 和 LLM terminal-tool loop 已与当前 main 的 Capability Hub Core、Agent Loop Run Control / Step Driver 合并；DeepSeek direct-chat wrapper 也已作为小边界抽取完成；Capability Runner Thin Shell / CLI first slices 和 Capability Search / Launch Plan first slice 也已完成；当前 full regression 是 `1389 passed, 5 skipped`。该合并不引入新依赖，不启动 real HTTP server，不打开 interactive shell / process supervisor / container / git worktree，也不把 aggressive product shell 整体合入。
...
- Capability Hub Core first slice 已合回 main，见 `docs/capability-hub-core-boundary-v0.2.md` 和 `docs/capability-hub-core-merge-readiness-review.md`。该 slice 来自 aggressive branch mainline merge review：不整体合并 `codex/spike-aggressive-dev`，也不复制 11708 行 `capability_hub.py`；当前只抽取最小 catalog core：`isotope_kernel.capability_catalog` 提供 `Capability` metadata、shelf visibility、minimal list / manifest / status 和三个 product-candidate built-ins（`artifact.review`、`external.snapshot.review`、`approval.tool.runner`）。DeepSeek tool-call / product-chat provider route 已通过 controlled-terminal branch 合入；DeepSeek direct-chat wrapper 只抽取为 `DeepSeekChatProvider` 小边界，使用 fake transport 测试，不执行工具、不写 kernel events。`ask`、`interactive`、self-evolution harness、study companion slices、diagnostics / pressure-test capabilities、workflow engine 和 product shell 继续留在 aggressive 或后续单独设计。
- Aggressive Remaining Code Review 已落地，见 `docs/aggressive-remaining-code-review-v0.md`。结论：`origin/spike/aggressive-dev` 是 application-layer 试验田，不是可直接 merge 的功能分支；剩余 `capability_hub.py` 是约一万行 god module，`self_evolution.py` 是 deterministic review harness 而非真实自主进化。后续只能 extract-only；推荐下一步是 `Capability Runner Thin Shell` 的 docs-only boundary，再按 TDD 做 red tests。
- Capability Runner Thin Shell first slice 已完成，见 `docs/capability-runner-thin-shell-boundary-v0.2.md`。当前新增 `isotope_kernel.capability_runner`：复用现有 `capability_catalog` 作为 source of truth，提供 `list / describe / status / run` 薄执行壳；第一批 `run` 只允许 `artifact.review`、`external.snapshot.review`、`approval.tool.runner` deterministic scenarios。unknown / diagnostic / experimental / provider-required / unallowlisted capability 在产生 side effect 前 fail closed；不复制 aggressive `capability_hub.py` / `self_evolution.py`，不打开 workflow engine、product shell、study companion、自我进化 harness 或 real provider runner。
- Capability Runner Thin Shell Closure Review 已落地，见 `docs/capability-runner-thin-shell-closure-review.md`。后续用户明确要求继续消化 aggressive 剩余代码后，主线已抽取最小 CLI first slice，见 `docs/capability-runner-cli-boundary-v0.2.md`：`python -m isotope_kernel.capability_runner` 支持低敏 `list / describe / status / run`，仍只调用现有 thin runner allowlist，不复制 aggressive `capability_hub.py`，不实现 `ask` / `interactive` / workflow engine / product shell / real provider runner。CLI 后的 intake refresh 见 `docs/aggressive-remaining-code-intake-refresh-v0.md`。Capability Search / Launch Plan first slice 已实现，见 `docs/capability-search-launch-plan-boundary-v0.2.md`：`CapabilityRunner.search_capabilities(...)` 做 deterministic catalog search；`CapabilityRunner.plan_capability_run(...)` 生成 low-sensitive no-side-effect launch plan。未来 LLM router 可以复用同一 launch plan contract，但当前不实现 LLM router、provider-backed runner、self-evolution 或 CLI `search` / `plan`。
...
- capability runner thin shell first slice complete / closed for now
- `src/isotope_kernel/capability_runner.py`
- `docs/capability-runner-thin-shell-boundary-v0.2.md`
- `docs/capability-runner-thin-shell-closure-review.md`
