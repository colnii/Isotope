...
- 最新 docs/status target：Restart Approval Resolution Context；当前已经 external review ready，见 `docs/post-external-review-checkpoint.md`、`docs/kernel-mainline-maintenance-mode.md` 和 `docs/mainline-idle-checkpoint.md`。外部 reviewer 入口是 `docs/external-review-package-v0.2.md`，它用一个短路径说明 Isotope 现在是 event-sourced agent kernel prototype、能跑哪些 demos、这些 demos 证明了哪些 kernel contracts、以及哪些 product / integration surfaces 仍 deliberately not implemented。主线仍按 conservative maintenance mode 管理；Tool Protocol first slice 已关闭为 model / event-shape slice，来自 aggressive-dev / review 的 `tool_invocation_runtime_missing`、`restart_write_helper_run_context_missing` 和 `restart_approval_resolution_context_missing` 均已由 bounded green slice 收口。仍不实现 plugin marketplace、remote tool、sandbox process、streaming output、public SDK、provider adapter、real HTTP、scheduler/process supervisor 或新依赖。application-layer prototype / aggressive branch 仍应先制造真实 friction，kernel mainline 只接收应用层或外部 review 已证明必要的 helper / boundary / replay / checkpoint / API ergonomics gap。
