...
- 最新 docs/status target：Policy Profile / Action Registry Versioning first green slice；registry/profile version basis metadata 已进入 proposal / decision / event payload / action read model，不实现 plugin / marketplace / policy DSL / migration framework。
...
- Kernel Gap Review Refresh 已落文档，见 `docs/kernel-gap-review-refresh-v0.2.md`。当前判断：agent / worker lifecycle、workspace substrate、workspace resource lifecycle、retry / cancel / supersede、HTTP facade、approval pause / resume、external ingestion boundary、artifact content read policy 和 policy profile / action registry versioning 都已足够 first-slice；仍 open 的 kernel-level gaps 是 retry / cancel / supersede runtime integration、worker handoff app composition、session / run lifecycle、error taxonomy、event schema registry、tool protocol 和 future real workspace substrate。Policy Profile / Action Registry Versioning first slice 已实现，见 `docs/policy-profile-action-registry-versioning-boundary-v0.2.md`；下一步推荐 closure review，不要直接开 real HTTP server、real LLM、provider adapter、memory query engine、filesystem sandbox / container 或第三个 app spike。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Policy Profile / Action Registry Versioning first slice 当前 complete；Next Suggested Batch 建议 `Policy Registry Version Basis Closure Review`。
...
- Policy Profile / Action Registry Versioning first green slice 已实现
...
- follow `docs/agent-task-queue.md` rolling batch mode；Policy Profile / Action Registry Versioning first slice 已完成。下一步推荐 `Policy Registry Version Basis Closure Review`；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine 或 filesystem/container substrate。
