...
- 最新 docs/status target：Event Schema Registry / Compatibility Green Slice；`src/isotope_kernel/event_schema.py` 已提供 static in-process `EventSchemaRegistry`，projector 会对 unknown `event_type` 和 unsupported payload `event_schema_version` fail closed。legacy/current known events 缺少 `event_schema_version` 时只通过 explicit registry compatibility mapping 解释；下一步是 closure review。当前仍不实现 schema migration framework、JSON Schema / protobuf dependency、plugin event types 或 remote registry。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Event Schema Registry / Compatibility green slice 当前 complete；Next Suggested Batch 建议 `Event Schema Registry / Compatibility Closure Review`。
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `986 passed` after Event Schema Registry / Compatibility green slice
...
- follow `docs/agent-task-queue.md` rolling batch mode；Event Schema Registry / Compatibility green slice 已 complete。下一步推荐 `Event Schema Registry / Compatibility Closure Review`；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine、plugin marketplace、policy DSL、schema migration framework、scheduler、process kill 或 filesystem/container substrate。
