...
- Capability Hub Core Boundary 已落文档，见 `docs/capability-hub-core-boundary-v0.2.md`。该 boundary 来自 aggressive branch mainline merge review：不整体合并 `codex/spike-aggressive-dev`，也不复制 11708 行 `capability_hub.py`；后续只从 `origin/main` 干净抽取最小 catalog core：`Capability` metadata、shelf visibility、minimal list / manifest / status 和少量 product-candidate registration。`ask`、`interactive`、LLM route、DeepSeek provider、self-evolution harness、study companion slices、diagnostics / pressure-test capabilities、workflow engine 和 product shell 继续留在 aggressive 或后续单独设计。
