...
- Capability Hub Core first slice 已完成并 ready to merge，见 `docs/capability-hub-core-boundary-v0.2.md` 和 `docs/capability-hub-core-merge-readiness-review.md`。该 slice 来自 aggressive branch mainline merge review：不整体合并 `codex/spike-aggressive-dev`，也不复制 11708 行 `capability_hub.py`；当前只从 `origin/main` 干净抽取最小 catalog core：`isotope_kernel.capability_catalog` 提供 `Capability` metadata、shelf visibility、minimal list / manifest / status 和三个 product-candidate built-ins（`artifact.review`、`external.snapshot.review`、`approval.tool.runner`）。`ask`、`interactive`、LLM route、DeepSeek provider、self-evolution harness、study companion slices、diagnostics / pressure-test capabilities、workflow engine 和 product shell 继续留在 aggressive 或后续单独设计。
