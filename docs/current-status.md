...
- 最新 docs/status target：Workspace Resource Lifecycle Boundary；本轮是 docs-only workspace lifecycle boundary，不改 `src` / `tests`。
...
- v0.1 developer demo 已按 `docs/demo/v0.1-demo-acceptance.md` accepted：acceptance anchor 当时依据是 `568 passed`、demo plain / JSON 本地可运行、editable install smoke 已覆盖、远端 GitHub Actions CI 已由网页确认通过；当前 baseline 已随 external snapshot review demo 更新为 `913 passed`。
...
- Kernel Gap Review Refresh 已落文档，见 `docs/kernel-gap-review-refresh-v0.2.md`。当前判断：agent / worker lifecycle、workspace substrate、retry / cancel / supersede、HTTP facade、approval pause / resume、external ingestion boundary 和 artifact content read policy 都已足够 first-slice；仍 open 的 kernel-level gaps 是 workspace resource lifecycle、policy profile / action registry versioning、retry / cancel / supersede runtime integration、worker handoff app composition、session / run lifecycle、error taxonomy、event schema registry 和 tool protocol。推荐下一步是 docs-only `Workspace Resource Lifecycle Boundary`，不要直接开 real HTTP server、real LLM、provider adapter、memory query engine、filesystem sandbox / container 或第三个 app spike。
- Workspace Resource Lifecycle Boundary 已落文档，见 `docs/workspace-resource-lifecycle-boundary-v0.2.md`。当前只定义 boundary：workspace 是 policy-bound execution resource，不是 agent identity；binding 和 lease 分离；candidate events 包括 `workspace.lease_created`、`workspace.bound`、`workspace.released`、deferred `workspace.release_failed` 和 optional `workspace.artifact_captured`；artifact capture 必须走 artifact / `ResourceRef` / provenance path，projector 仍不能读 workspace filesystem。下一步建议只写 red tests，不实现真实 filesystem / container / git worktree / remote executor / cleanup scheduler。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Workspace Resource Lifecycle Boundary 当前 complete；Next Suggested Batch 建议 `Workspace Resource Lifecycle Red Tests`，red-only by default。
...
- workspace resource lifecycle boundary defined
- `docs/workspace-resource-lifecycle-boundary-v0.2.md`
- recommendation: `Workspace Resource Lifecycle Red Tests` before implementation
...
- demo acceptance evidence includes local `568 passed` at the v0.1 acceptance anchor; current mainline baseline is `913 passed` after external snapshot review demo
...
- v0.2 acceptance evidence includes `735 passed` at the tag anchor, v0.1 demo plain / JSON success, v0.2 demo plain / JSON success, configured CI smoke, no `x_agent.*` imports, and `/home/lumber/Github/x-agent` untouched; current mainline baseline is `913 passed`
...
- follow `docs/agent-task-queue.md` rolling batch mode；Workspace Resource Lifecycle Boundary 已完成。下一步推荐 `Workspace Resource Lifecycle Red Tests`，red-only by default；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine 或 filesystem/container substrate。
- Workspace Substrate Boundary v0.2 first slice complete；Workspace Resource Lifecycle Boundary 已定义 lease / release / artifact-capture contract，后续如继续 workspace 应先写 red tests
