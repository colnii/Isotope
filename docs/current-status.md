...
- 最新 docs/status target：Kernel Gap Review Refresh；本轮是 docs-only kernel gap refresh，不改 `src` / `tests`。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Kernel Gap Review Refresh 当前 complete；Next Suggested Batch 建议 `Workspace Resource Lifecycle Boundary`，docs-only by default。
...
- recommendation: `Workspace Resource Lifecycle Boundary` before the next app spike
...
- follow `docs/agent-task-queue.md` rolling batch mode；Kernel Gap Review Refresh 已完成。下一步推荐 `Workspace Resource Lifecycle Boundary`，docs-only by default；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine 或 filesystem/container substrate。
