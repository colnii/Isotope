...
- 最新 docs/status target：Retry / Cancel / Supersede Runtime Integration Green Slice；`InProcessServer.request_retry(...)` / `request_cancel(...)` / `request_supersede(...)` 已实现最小 helper，仍不实现 scheduler / process kill / real concurrency。
...
- agent task queue 已落文档，见 `docs/agent-task-queue.md`。后续默认使用 rolling batch mode：session timebox 为 45-60 min，先读 `docs/current-status.md`、`docs/v0.2-roadmap.md` 和 `docs/agent-task-queue.md`，执行 Current Batch；如果 clean 且时间充足，可以在更新 queue、完成验证和 commit / push 后把 Next Suggested Batch 提升为 Current Batch 继续执行。不要为了凑时间做未列出的任务，遇到 stop condition 或需要产品 / 用户判断时必须停。Retry / Cancel / Supersede Runtime Integration helper slice 当前 green；Next Suggested Batch 建议 `Retry / Cancel / Supersede Runtime Integration Closure Review`。
...
- Retry / Cancel / Supersede Runtime Integration helper slice 已 green，见 `docs/retry-cancel-supersede-runtime-integration-boundary-v0.2.md`。当前 `InProcessServer.request_retry(...)`、`request_cancel(...)` 和 `request_supersede(...)` 提供最小 in-process runtime helpers：request append canonical R/C/S events；cancel 是 logical cancel，不是 process kill；retry / supersede 使用 replacement proposal / execution identity；request 不原地修改 old action / execution，不引入 hidden scheduler state。
...
- follow `docs/agent-task-queue.md` rolling batch mode；Retry / Cancel / Supersede Runtime Integration helper slice 已 green。下一步推荐 `Retry / Cancel / Supersede Runtime Integration Closure Review`；不要直接扩大 Track F provider / webhook / HTTP ingestion API，也不要直接开第三个 app spike、real HTTP server、real LLM、memory query engine、plugin marketplace、policy DSL、scheduler、process kill 或 filesystem/container substrate。
