...
- Track C boundary doc 已落地：`docs/artifact-content-read-policy-v0.2.md`。它只定义 controlled full-content retrieval 边界，不实现 full content API。
- 当前测试基线：`647 passed`。
...
14. Track A 当前已 effectively complete / closed for now；`docs/v0.2-next-track-selection.md` 已选择 Artifact Content Read Policy / Controlled Full-Content Retrieval 作为下一默认 track，`docs/artifact-content-read-policy-v0.2.md` 已定义其 docs-only boundary。real server boundary design 只有在 Track A 被显式 reopened 时再做。
...
- Artifact Content Read Policy / Controlled Full-Content Retrieval red tests。
