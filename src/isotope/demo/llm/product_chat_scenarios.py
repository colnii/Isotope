...
from ...features.chat.flow import submit_llm_product_chat_user_message_with_readiness_check
...
def _run_llm_product_chat_app_entry_demo(root: Path) -> dict[str, Any]:
    ...
    blocked_response = submit_llm_product_chat_user_message_with_readiness_check(
        app,
        run_id,
        readiness_check=_demo_product_chat_blocked_readiness_check(),
    ...
    blocked_no_side_effects = (
        ...
        and blocked_body.get("status") == "blocked_by_readiness_check"
        and provider.calls == []
        and runner.calls == []
        and after_blocked_events == before_blocked_events
    )

    ready_readiness_check = _demo_product_chat_ready_readiness_check()
    ready_response = submit_llm_product_chat_user_message_with_readiness_check(
        app,
        run_id,
        readiness_check=ready_readiness_check,
        system_message="Use the product-chat app entry.",
    ...
    app_entry_readiness_check_ok = (
        blocked_no_side_effects
        and ready_readiness_check.get("ready") is True
    ...
    return {
        ...
        "app_entry_readiness_check_ok": app_entry_readiness_check_ok,
        ...
        "blocked_readiness_check_category": blocked_body.get("readiness_check", {}).get("category"),
        "ready_readiness_check_ready": ready_readiness_check.get("ready") is True,
