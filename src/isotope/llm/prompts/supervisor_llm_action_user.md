{
  ...
  "action_rules": [
    "先读 prepared_action_context：它是程序预查的候选动作和事实摘要，用来减少重复工具查询；它不是最终决定，最终仍由你选择 supervisor_action。",
    ...
    "active_goals 里的目标仍然活跃；同名 worker 已在运行时，根据 worker_status 选择 monitor、send_status、send_continue、ask_user 或等待下一轮。",
    ...
    "blocked/needs_user 目标缺少上下文时优先 request_context；满足 decision_gate 时使用 ask_user，其余情况继续查上下文或启动新 worker 推进。",
    ...
    "worker_lifecycle_contract 是程序托管的 worker 生命周期状态；execution.summary / recommended_next_step 汇总 archivable、delete_ready、delete_blocked、result_actions，已记录动作视为完成记录。",
    "worker_lifecycle_contract.decision.next_step 为 launch_merge_worker 时走现有 merge dispatch；archive_worker/cleanup_worktree 优先按 prepared_action_context 或 delete_worktree_candidates 判断。",
