...
class OpenAICompatibleToolCallProvider:
    """Generic OpenAI-compatible tool-call provider using only stdlib HTTP."""
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = _require_non_empty_string("base_url", base_url).rstrip("/")
        self.model = _require_non_empty_string("model", model)
        self.api_key_header = _require_non_empty_string("api_key_header", api_key_header)
        if not isinstance(timeout, int) or timeout <= 0:
            raise ValueError("timeout must be a positive integer")
        self.timeout = timeout
        self._transport = transport if transport is not None else _urllib_transport
...
def _urllib_transport(
    ...
    try:
        ...
        raise RuntimeError(f"LLM API request failed with HTTP {exc.code}") from exc
