...
The spike creates a small topmost `WS_POPUP` window, clips it with `SetWindowRgn`, paints a teal circular orb using GDI, and returns `HTCAPTION` for hit tests inside the circle so the orb can be dragged by holding the visible circle.
