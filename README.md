...
- Demo trace mode: `--trace` is available for `v0.2`, `approval-tool-runner`, `artifact-review`, `external-snapshot-review`, and `agent-loop-friction` to print human-readable runtime steps; default plain output and `--json` remain compatible, and trace does not expose artifact full content.
