...
当前状态：`v0.1-demo` developer demo 已完成并打 tag；当前本地 baseline 是 `669 passed`。Track A: HTTP API Minimal Surface 已 effectively complete / closed for now；Track C artifact content retrieval boundary 第一批已实现，详细状态见 [docs/current-status.md](docs/current-status.md)。
...
- Controlled artifact content retrieval boundary through structured `ResourceRef`, explicit grants, caller context, and purpose.
