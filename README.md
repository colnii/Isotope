...
- Tool Protocol Boundary docs-only: defines grants-only tool invocation, tool result / error shape, artifact / `ResourceRef` handoff, provenance, budget and registry relationship; it does not implement plugin marketplace, remote tools, sandboxed process, streaming output, or public SDK.
