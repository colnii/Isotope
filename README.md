...
当前 deferred 边界：real LLM、`ActionTypeRegistry`、memory write、external ingestion / `ImportedSnapshot`、public checkpoint API / HTTP endpoint、checkpoint save trigger / automatic checkpoint scheduling、CheckpointService、signature / MAC / key management、event prefix digest、checkpoint migration / version negotiation、SSE、auth、multi-agent concurrency、real HTTP API。`InProcessServer` read path 已可选使用 checkpoint-assisted rebuild，但 server 仍不直接读取或解释 checkpoint state，也不创建 / 保存 checkpoint。
