...
- checkpoint retention / compaction 已有 design note；当前仍是 latest-only checkpoint storage，没有 history / GC / automatic scheduling。retention / compaction 只能处理 checkpoint blobs，不能删除、重写、压缩、裁剪 canonical event log；checkpoint 删除后仍必须能从 event log full rebuild。
