...
- Demo trace mode: `--trace` is available for `v0.2`, `approval-tool-runner`, `artifact-review`, `external-snapshot-review`, `agent-loop-friction`, `agent-loop-planner-friction`, `agent-loop-planner-matrix`, `agent-loop-planner-restart-pause`, and `agent-loop-planner-io-validator` to print human-readable runtime steps; default plain output and `--json` remain compatible, and trace does not expose artifact full content.
