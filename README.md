...
Capability Runner Thin Shell closure review 见 [docs/capability-runner-thin-shell-closure-review.md](docs/capability-runner-thin-shell-closure-review.md)。后续按用户要求继续消化 aggressive 剩余代码时，已抽取最小 CLI first slice，见 [docs/capability-runner-cli-boundary-v0.2.md](docs/capability-runner-cli-boundary-v0.2.md)：`python -m isotope_kernel.capability_runner` 现在支持低敏 `list / describe / status / run`，但仍不复制 aggressive `capability_hub.py`、不做 `ask` / `interactive` / workflow engine。CLI 后的剩余代码 intake refresh 见 [docs/aggressive-remaining-code-intake-refresh-v0.md](docs/aggressive-remaining-code-intake-refresh-v0.md)。Capability Search / Launch Plan first slice 已实现，见 [docs/capability-search-launch-plan-boundary-v0.2.md](docs/capability-search-launch-plan-boundary-v0.2.md)：现在 Python API 可做 capability search 和 launch-plan preflight，但 CLI `search` / `plan`、provider runner、LLM router 和 self-evolution 仍未实现。
...
.venv/bin/python -m isotope_kernel.demo --scenario agent-loop-planner-io-validator
.venv/bin/python -m isotope_kernel.demo --scenario agent-loop-planner-io-validator --trace
.venv/bin/python -m isotope_kernel.demo --scenario agent-loop-planner-io-validator --json
...
.venv/bin/python -m isotope_kernel.capability_runner list --json
.venv/bin/python -m isotope_kernel.capability_runner run artifact.review --json
```
...
- Capability Runner Thin Shell / CLI first slices: `isotope_kernel.capability_runner` can list/describe/status catalog capabilities and run three allowlisted deterministic product candidates from Python API or `python -m isotope_kernel.capability_runner`, without copying aggressive hub or constructing providers.
...
- DeepSeek direct chat provider wrapper: `DeepSeekChatProvider` offers a stdlib-only OpenAI-compatible `/chat/completions` boundary for `deepseek-v4-flash`; tests use fake transport, and the wrapper does not execute tools or write kernel events.
...
- Capability Hub Core merge readiness: [docs/capability-hub-core-merge-readiness-review.md](docs/capability-hub-core-merge-readiness-review.md)
